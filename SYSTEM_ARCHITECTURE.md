...
## 1. Khởi tạo mạng & Đăng nhập (TCP)
Quy trình thiết lập kết nối đáng tin cậy giữa Client và Server để định danh người dùng.

### 📜 Luồng thực thi:
1. **Người dùng ấn Đăng nhập:**
   - Class: `LoginController.java`
   - Method: `onLoginClick()`
   - Hành động: Client mở một Socket TCP tới Server thông qua lệnh `connection.connect(host, port)` (lớp `TCPConnection`). Sau đó gửi một đối tượng `Message` với `type = MessageType.LOGIN` chứa JSON payload (`username`, `password`).

2. **Server tiếp nhận kết nối:**
   - Class: `ServerMain.java`
   - Hành động: Liên tục `serverSocket.accept()`. Khi có Client tới, tạo ra một luồng `ClientHandler` mới để phục vụ riêng cho Client này.

3. **Xử lý đăng nhập tại Server:**
   - Class: `ClientHandler.java`
   - Method: `run()` -> đọc dòng JSON -> gọi `handleMessage(msg)` -> chuyển hướng vào `handleLogin(payload)`.
   - Hành động:
     - Truy vấn DB: `UserRepo.findByUsername(username)`.
     - Xác thực mật khẩu: Dùng `org.mindrot.jbcrypt.BCrypt.checkpw(...)`.
     - Đăng ký phiên hoạt động: Gọi `ServerManager.registerUser(u, this)` để đưa `ClientHandler` này vào danh sách Online.
     - Trả về: `sendMessage(new Message(MessageType.LOGIN_RESP, ...))` về lại Client.

---

## 2. Giao tiếp Text Chat (TCP)
Xử lý các tin nhắn văn bản, yêu cầu tính toàn vẹn dữ liệu và lưu trữ lịch sử.

### 📜 Luồng thực thi:
1. **Client gửi tin nhắn:**
   - Class: `MainController.java`
   - Method: `onSendMessage()`
   - Hành động: Bắt sự kiện Enter từ UI, tạo payload chứa `receiverId` (hoặc `groupId`), `content`. Gọi `connection.sendMessage()` với type là `SEND_DM` hoặc `SEND_GROUP_MSG`.

2. **Server định tuyến tin nhắn:**
   - Class: `ClientHandler.java`
   - Method: `handleChatMessage()`
   - Hành động:
     - Lưu vào CSDL trước để lưu lịch sử (thông qua `MessageRepo.createDM(...)`).
     - Lấy Socket của người nhận thông qua ID: `ServerManager.sendToUser(receiverId, notifyMsg)` (đối với Chat 1-1). Nếu là Chat Group, truy vấn danh sách thành viên `GroupRepo.getGroupMembers()` và gửi vòng lặp bằng `ServerManager.sendToUsers(...)`.

---

## 3. Truyền Tải File (TCP Chunking)
Xử lý việc gửi file lớn qua mạng mà không làm treo hệ thống hay tràn RAM (Out of Memory).

### 📜 Luồng thực thi:
1. **Client băm nhỏ file (Upload):**
   - Class: `MainController.java`
   - Method: `onUploadFile()` và `sendChunks()`
   - Hành động:
     - Bước 1: Gửi lệnh `UPLOAD_FILE_INIT` mang thông tin tên file, kích thước. Server trả về `UPLOAD_FILE_RESP` cấp một `fileId`.
     - Bước 2 (`sendChunks`): Client mở luồng đọc byte từ file, cứ mỗi 64KB (hoặc kích thước cố định) lại mã hóa thành mảng byte Base64, gói vào lệnh `FILE_CHUNK` gửi lên TCP.

2. **Server ghép mảnh File (Assemble):**
   - Class: `ClientHandler.java`
   - Method: `handleFileChunk(payload)`
   - Hành động:
     - Đọc mảng byte chunk, mở file ở chế độ nối `StandardOpenOption.APPEND` và ghi vào đĩa (`server_storage/`).
     - Kiểm tra liên tục: `if (Files.size(path) >= meta.getFileSize())`. Nếu bằng kích thước khai báo, tức là đã hoàn tất.
     - Broadcast báo hiệu hoàn tất `FILE_UPLOAD_NOTIFY` cho các Client khác trong phòng Chat.

---

## 4. Cuộc gọi Thoại / Video & Screen Share (TCP Signaling + UDP Multicast Media)
Phần phức tạp nhất của hệ thống, kết hợp hai giao thức mạng để đạt được tính thời gian thực (Real-time).

### Giai đoạn 4.1: Bắt tay tín hiệu (Signaling qua TCP)
1. **Yêu cầu gọi:** Client A bấm nút gọi, `MainController` gửi lệnh `CALL_REQUEST` (TCP).
2. **Server làm mai:** `ClientHandler` tại Server nhận được, chuyển lệnh này thành tín hiệu đổ chuông sang Client B.
3. **Đồng ý & Phân bổ IP:** Khi B ấn nghe (`CALL_ACCEPT`), Server sinh ra một địa chỉ IP Multicast (ví dụ `239.1.1.1`) gửi kèm trong thông điệp `STREAM_STARTED` trả về cho cả A và B.

### Giai đoạn 4.2: Truyền Media thực tế (Qua UDP)
*Bắt đầu từ lúc này, Server TCP không còn tham gia truyền luồng nữa để giảm tải.*

1. **Thu thập dữ liệu phần cứng (Client Gửi):**
   - Class: `WebcamCaptureThread` / `AudioCaptureThread` / `ScreenCaptureThread`
   - Method: Vòng lặp `run()` liên tục chụp ảnh Webcam (JPEG) hoặc đọc micro (PCM).
   - Hành động: Ném mảng byte thô vào `sender.sendWebcam(bytes)`.

2. **Phân mảnh gói tin UDP (UDP Fragmentation):**
   - Class: `edu.vku.lancord.client.udp.UDPStreamSender`
   - Method: `sendFragmented(mediaType, data, isKeyframe)`
   - Hành động: 
     - Do mảng byte ảnh JPEG quá to so với 1 gói tin mạng UDP chuẩn (chỉ khoảng 1400 bytes). Method này chia nhỏ bức ảnh bằng công thức `Math.ceil(data.length / MAX_PAYLOAD)`.
     - Với mỗi mảnh, đính kèm một Header 12 byte tự định nghĩa thông qua `UDPHeader.encode()`. Header này chứa thông tin: `MEDIA_TYPE`, `SEQ_ID` (Id bức ảnh), `CHUNK_INDEX` (Thứ tự mảnh), `TOTAL_CHUNKS` (Tổng số mảnh).
     - Đẩy mảnh qua `MulticastSocket.send(...)`.

3. **Lắp ráp & Phát lại (Client Nhận):**
   - Class: `edu.vku.lancord.client.network.UDPStreamReceiver`
   - Method: `start()`
   - Hành động:
     - Thread liên tục lắng nghe `socket.receive(packet)`.
     - Phân tích 12 byte đầu (`UDPHeader`) để biết được số lượng chunk.
     - Đưa mảnh vỡ vào một `Map<Integer, byte[]>` (hoặc lớp `FragmentBuffer`) đóng vai trò chờ ghép.
     - Kiểm tra `if (chunkMap.size() == totalChunks)`: Khi đã thu thập đủ các mảnh ghép của 1 khung hình, tiến hành nối chúng lại thành 1 mảng byte hoàn chỉnh.
     - Biến mảng byte thành JavaFX `Image` (nếu là video) hoặc đưa vào `SourceDataLine` (nếu là audio) để phát lên UI. Chạy callback `onFrameReceived.accept(image)`.

---
**Tổng kết:** Kiến trúc sử dụng **TCP** làm "bộ não" (Control Plane) đảm bảo điều phối chính xác và **UDP Multicast** làm "cơ bắp" (Data Plane) dồn toàn lực vào tốc độ truyền dẫn. Kiến trúc này mang lại khả năng mở rộng tốt và hạn chế thắt cổ chai (bottleneck) ở máy chủ trung tâm.
